    ...
    <basic-container>
      <div class="shipowner_title">
        ...
        <el-table
          ...
          style="width: 100%">
          <el-table-column
            prop="recruit.contactName"
            label="联系人"
          >
          </el-table-column>
          <el-table-column
            prop="recruit.positionId"
            label="应聘岗位"
          >
          <template slot-scope="scope">
             {{ changePosition(scope.row.recruit.positionId,'position') }}
          </template>
          </el-table-column>
          <el-table-column
            label="年龄要求"
          >
          <template slot-scope="scope">
             {{ changePosition(scope.row.recruit.ageRequire,'age') }}
          </template>
          </el-table-column>
          <el-table-column
            label="作业方式"
          >
          <template slot-scope="scope">
             {{ changePosition(scope.row.recruit.workMode,'worktype') }}
          </template>
          </el-table-column>
          <el-table-column
            prop="recruit.salary"
          ...
          <el-table-column
            prop="recruit.createTime"
            label="更新时间"
          >
          </el-table-column>
          <el-table-column label="操作" width="180">
            <template slot-scope="scope">
              <el-button type="text" icon="el-icon-delete" size="mini" @click="handleDel(scope.row.applyId)">删除
              </el-button>
              <el-button type="text" size="mini" @click="handleView(scope.row.recruit.recruitId)">查看
              </el-button>
            </template>
          </el-table-column>
        </el-table>
...
<script>
import { getPage, deleteById } from '@/api/post/myJob'
  ...
  methods: {
    ...
    currentChange (val) {
      ...
      this.getData()
        ...
        // 查看
    handleView (val) {
      this.$router.push({name: 'myJobDetail',query:{ see: val }})
    },
    // 获取列表数据
    getData () {
