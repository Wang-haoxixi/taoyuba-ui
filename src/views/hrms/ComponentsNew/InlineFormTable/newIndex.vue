      ...
      <el-table-column :label="item.label" :width="item.width" v-for="(item, idx) in columns" :key="idx">
        <template slot-scope="scope">
          <template v-if="scope.row.editable">
            ...
            <iep-upload-select v-else-if="item.type === 'file'" size="mini" v-model="scope.row[item.prop]" :type="true" @input="getUrl()"></iep-upload-select>
            ...
            <el-autocomplete :maxlength="100" size="mini" v-else-if="item.prop ==='shipname'" v-model="scope.row[item.prop]" :fetch-suggestions="querySearchAsync" :placeholder="item.label" @select="handleSelect" clearable></el-autocomplete>
          ...
          <template v-else>
            ...
            <iep-upload-select v-else-if="item.type === 'file'" size="mini" :value="scope.row[item.prop]" disabled :type="true"></iep-upload-select>
...
export default {
  ...
  methods: {
    ...
    async querySearchAsync (queryString, cb) {
      if (queryString !== '') {
        await findMyship(queryString).then(data =>{
          this.shipList = data.data.data
          this.restaurants.push({
            value: data.data.data.shipName,
          })
        })
