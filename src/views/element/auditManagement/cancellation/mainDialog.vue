        ...
        option: [
          {
            column: [
              ...
              {label: '重点领域分类', prop: 'areaTypeList', type: 'select', multiple: true, dicData: getDic('AREA_TYPE'), span: 12},
              ...
              {label: '关联资源代码', prop: 'rescode', span: 12},
              {label: '发布日期', prop: 'createTime', span: 12},
            ],
          },
        ],
      }
  ...
  methods: {
    ...
    handleAuditPass () {
      this.$confirm('是否审核通过?', '审核', {
          ...
          this.$emit('auditPass', this.formData.id)
        }).catch(() => {})
    },
    // 审核驳回
    handleAuditReject () {
      this.$prompt('请输入驳回理由', '驳回', {
        ...
        this.$emit('auditReject', {id: this.formData.id, reason: value})
      }).catch(() => {})
    },
  },
  created () {
