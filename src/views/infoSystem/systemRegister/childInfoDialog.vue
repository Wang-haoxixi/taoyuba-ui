  ...
  methods: {
    selectionChange2 (val) {
      this.tableList.map((item)=>{
        ...
        // }
        // for(let queryItem of this.queryData){
        //   if(queryItem.id == item.id){
        //     this.queryData.splice(this.queryData)
        //   }
        // }

        this.queryData.map((queryItem,index) => {
          if(queryItem.id == item.id){
        ...
        if (data.code === 0) {
          ...
          this.$nextTick(()=>{
            let currentTableList = this.tableList.filter((item)=>{
              for(let oldItem of oldQueryData){
