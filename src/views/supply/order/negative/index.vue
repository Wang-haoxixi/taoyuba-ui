...
import { getPageByNegative } from '@/api/supply/order'
...
export default {
  ...
  data () {
    return {
      searchOption,
      tableOption,
      excelUrl: '/gvs/excel/negative_export',
    }
  },
  methods: {
    // 列表
    getList () {
      ...
      getPageByNegative(this.listQuery).then((res) => {
    ...
    handleExportQuery () {
      ...
      let next = new Promise((resolve) => {
        download({url: this.excelUrl, title: '负能清单', data: this.ignoreData(this.listQuery)})
        resolve()
      })
      next.then(() => {
        this.loading = false
