...
import { processingData } from '../confirm/const/index'
import { supplyResData, getConfirmDetailData } from '@/api/supply/material'
      ...
      return {
        ...
        column: [{
            label: '需求情况', prop: '', align: 'center', children: [
              { label: '标记责任部门', prop: 'deptId', align: 'center', type: 'select', dicData: getDept() },
              { label: '需求部门', prop: 'demandDeptId', align: 'center', type: 'select', dicData: getDept() },
              { label: '事项名称', prop: 'matName', align: 'center' },
              { label: '材料名称', prop: 'materialName', align: 'center' },
              { label: '材料类型', prop: 'materialType', align: 'center', type: 'dic', dicData: getDic('SUPPLU_MATERIAL_TYPE') },
            ],
          }, {
            label: '认责情况', prop: '', align: 'center', children: [
              { label: '状态', prop: 'menu', solt: true, align: 'center' },
              { label: '确认数源部门', prop: 'confirmDeptId', align: 'center', type: 'select', dicData: getDept() },
            ],
          },
        ],
      }
  ...
  methods: {
    ...
    handleDetail (row) {
      getConfirmDetailData(row.id).then((res) => {
        ...
        this.formData = processingData(res.data.data)
        console.log("formData: ", this.formData)
        this.openDialog('mainDialog')
      })
    },
    buttonList (row) {
      let name = ['待确认', '已确认', '拒绝', '撤销审核', '已撤回']
