...
import { getApplicationReviewPage, getApplicationFormById } from '@/api/cbs/application_form'
...
export default {
  ...
  computed: {
    ...
    searchOption () {
      return [
        { label: '材料名称', prop: 'materialName', type: 'input', placeholder: '请输入材料名称' },
      ]
    },
  },
  created () {
  ...
  methods: {
    ...
    load (pageOption = this.pageOption, params = this.params) {
      ...
      getApplicationReviewPage({ ...params, ...pageOption, auditState: 2 }).then(res => {
        const { data } = res
        this.loadTable(data)
      })
    },
